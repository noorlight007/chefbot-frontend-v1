...
import { useUpdatePromotionInfoMutation } from "@/redux/reducers/promotions-reducer";
import {
...
const promotionSchema = (t: ReturnType<typeof useTranslations>) =>
  z.object({
    uid: z.string(),
    title: z.string().min(1, t("errors.titleRequired")),
    message_template: z.string().min(1, t("errors.messageTemplateRequired")),
    ...
    reward: z.object({
      ...
      label: z
        .string()
        .min(1, t("errors.rewardLabelInvalid"))
        .max(24, t("errors.rewardLabelInvalid")),
    }),
    organization: z.string().min(1, t("errors.restaurantRequired")),
    valid_from: z.string().min(1, t("errors.validFromRequired")),
    valid_to: z.string().min(1, t("errors.validToRequired")),
    trigger: z.object({
      uid: z.string(),
      type: z.enum([
        "YEARLY",
      ...
      inactivity_days: z.number().min(1).optional(),
...
const UpdatePromotionModal: React.FC<UpdatePromotionModalProps> = ({
  ...
  const rewardTypeOptions = [
    { value: "DRINK", label: t("dropdownValues.rewardTypeOptions.DRINK") },
    { value: "DESSERT", label: t("dropdownValues.rewardTypeOptions.DESSERT") },
    {
      value: "DISCOUNT",
      label: t("dropdownValues.rewardTypeOptions.DISCOUNT"),
    },
    { value: "CUSTOM", label: t("dropdownValues.rewardTypeOptions.CUSTOM") },
  ];

  const YearlyCategory = [
    {
      value: "ANNIVERSARY",
      label: t("dropdownValues.yearlyCategoryOptions.ANNIVERSARY"),
    },
    {
      value: "BIRTHDAY",
      label: t("dropdownValues.yearlyCategoryOptions.BIRTHDAY"),
    },
  ];

  const triggerTypeOptions = [
    { value: "YEARLY", label: t("dropdownValues.triggerTypeOptions.YEARLY") },
    {
      value: "MENU_SELECTED",
      label: t("dropdownValues.triggerTypeOptions.MENU_SELECTED"),
    },
    {
      value: "INACTIVITY",
      label: t("dropdownValues.triggerTypeOptions.INACTIVITY"),
    },
    {
      value: "RESERVATION_COUNT",
      label: t("dropdownValues.triggerTypeOptions.RESERVATION_COUNT"),
    },
  ];

  const organizationUid =
    typeof promotion.organization === "string"
      ? promotion.organization
      : (promotion.organization as { uid: string }).uid;

  const [updatePromotionInfo, { isLoading: isUpdating }] =
  ...
  const form = useForm<PromotionFormData>({
    ...
    defaultValues: {
      ...
      trigger: {
        ...promotion.trigger,
        menus: Array.isArray(promotion.trigger.menus)
          ? promotion.trigger.menus.map((menu: string | { uid: string }) =>
              typeof menu === "string" ? menu : menu.uid,
            )
          : [],
      },
    },
  ...
  const messageTemplates = React.useMemo(() => {
    ...
    return messageTemplatesData?.results?.map(
      (template: { name: string; uid: string }) => ({
        value: template.uid,
        label: template.name,
      }),
    );
  ...
  const handleSubmit = async (values: PromotionFormData) => {
    try {
      await updatePromotionInfo({ id: promotion.uid, data: values }).unwrap();
      toast.success(t("success.promotionUpdated"));
      onClose();
    } catch (error: any) {
      ...
      if (error?.status === 400 && error?.data) {
        ...
        if (errors.reward?.label) {
          form.setError("reward.label", {
            type: "manual",
            message: errors.reward.label[0] || t("failure.rewardLabel"),
          });
          toast.error(errors.reward.label[0] || t("failure.rewardLabel"));
        }
        ...
        toast.error(t("failure.promotionUpdate"));
      }
    }
  };
  ...
  if (isUserLoading) {
    return (
      <div className="p-8 text-center text-gray-600">{t("loading.user")}</div>
    );
  }

  return (
    <div className="mx-auto max-w-full rounded-xl bg-white p-8 shadow-lg">
      <div className="mb-8 border-b border-gray-200 pb-4">
        <h2 className="text-3xl font-bold text-gray-900">
          {t("updateButton")}
        </h2>
        <p className="mt-2 text-sm text-gray-600">{t("editDescription")}</p>
      </div>
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-700">
              {t("restaurantLabel")} <span className="text-red-500">*</span>
            </label>
            <Dropdown
        ...
        <div className="space-y-2">
          ...
          {messageContentVariables ? (
            <div className="mt-2 text-xs text-gray-600">
              <div className="mb-1 font-medium">{t("availableVariables")}</div>
            ...
            <div className="mt-2 text-xs text-gray-600">{t("noVariables")}</div>
          )}
        ...
        <div className="rounded-lg bg-gray-50 p-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">
                {t("rewardTypeLabel")} <span className="text-red-500">*</span>
              </label>
              <Dropdown
            ...
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">
                {t("rewardLabelLabel")} <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                className="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
                placeholder={t("rewardLabelPlaceholder")}
                maxLength={24}
                {...form.register("reward.label")}
              />
              <p className="text-xs text-gray-500">
                {t("characterCount", {
                  count: form.watch("reward.label")?.length || 0,
                })}
              </p>
        ...
        <div className="rounded-lg bg-gray-50 p-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">
                {t("validFromLabel")} <span className="text-red-500">*</span>
              </label>
              <input
                type="date"
                className="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
            ...
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">
                {t("validToLabel")} <span className="text-red-500">*</span>
              </label>
              <input
                type="date"
        ...
        <div className="rounded-lg bg-gray-50 p-6">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">
                {t("triggerTypeLabel")} <span className="text-red-500">*</span>
              </label>
              <Dropdown
            ...
            {triggerType === "YEARLY" && (
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-gray-700">
                  {t("yearlyCategoryLabel")}{" "}
                  <span className="text-red-500">*</span>
                </label>
                <Dropdown
                  options={YearlyCategory}
                  placeholder={t("yearlyCategoryPlaceholder")}
                  onChange={(value) =>
                    form.setValue(
            ...
            {triggerType === "YEARLY" && yearlyCategory && (
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-gray-700">
                  {t("daysBeforeLabel")} <span className="text-red-500">*</span>
                </label>
                <input
                  type="number"
                  onWheel={(e) => e.currentTarget.blur()}
                  className="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
                  placeholder={t("daysBeforePlaceholder")}
            ...
            {triggerType === "MENU_SELECTED" && (
              <div className="space-y-2">
                ...
                {isMenuLoading ? (
                  ...
                  <Dropdown
                    options={menuOptions}
                    placeholder={t("menuSelectionPlaceholder")}
                    onChange={(value) =>
            ...
            {triggerType === "INACTIVITY" && (
              <div className="space-y-2">
                ...
                <input
                  ...
                  onWheel={(e) => e.currentTarget.blur()}
                  className="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
                  placeholder={t("inactivityDaysPlaceholder")}
            ...
            {triggerType === "RESERVATION_COUNT" && (
              <div className="space-y-2">
                ...
                <input
                  ...
                  onWheel={(e) => e.currentTarget.blur()}
                  className="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
                  placeholder={t("minimumCountPlaceholder")}
        ...
        <div className="flex justify-end space-x-4 border-t border-gray-200 pt-6">
          ...
          <button
            type="submit"
            className="rounded-lg bg-sidebar-accent px-6 py-2.5 text-sm font-medium text-white transition-colors"
            disabled={isUpdating}
          >
